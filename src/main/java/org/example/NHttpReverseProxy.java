...
public class NHttpReverseProxy {
    public static void main(String[] args) throws Exception {
        ...
        /* Backend Address*/
        URI uri = new URI(prop.getProperty("server"));
        /*Reverse Proxy port*/
        ...
        IOReactorConfig config = IOReactorConfig.custom()
                .setIoThreadCount(Integer.parseInt(prop.getProperty("ioReactorThreads")))
                .setSoTimeout(Integer.parseInt(prop.getProperty("timeoutSo")))
                .setConnectTimeout(Integer.parseInt(prop.getProperty("timeoutConnection")))
                .build();
        ...
        connPool.setMaxTotal(Integer.parseInt(prop.getProperty("MaxTotal")));
        connPool.setDefaultMaxPerRoute(Integer.parseInt(prop.getProperty("DefaultMaxPerRoute")));
        ...
        if (port == 8443) {
            ...
            char[] secret = prop.getProperty("keyStorePassword", "changeit").toCharArray();
            //ClassLoader cl = NonBlockingReverseProxy.class.getClassLoader();
            //URL url = cl.getResource(keystoreFile);
            URL url = new URL("file://" + prop.getProperty("keyStore"));
            if (url == null) {
                System.out.println("KeyStore [" + url.getFile() + "] not found.");
                System.exit(1);
            }
            ...
            listeningEventDispatch = new DefaultHttpServerIODispatch(
                    serviceHandler, sslcontext, null, ConnectionConfig.DEFAULT);
            System.out.println("HTTPS listener started on port : " + port);
        } else {
            //connFactory = new DefaultNHttpServerConnectionFactory(
            //        ConnectionConfig.DEFAULT);
            listeningEventDispatch = new DefaultHttpServerIODispatch(
                    serviceHandler, ConnectionConfig.DEFAULT);
            System.out.println("HTTP listener started on port : " + port);
        }
